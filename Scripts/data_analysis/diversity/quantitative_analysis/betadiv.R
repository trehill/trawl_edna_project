#Making a dataset for analysis on betadiversity 
...
trawl_df <-  read.csv(here::here("Processed_data",
                             "trawl",
                             "datasets",
                             "fulldatasettrawlweight.csv"), #has weight
                  head=TRUE)
...
eDNA <- select(eDNA_df, c('LCT', 'set_number' ))
eDNA <- distinct(eDNA)

trawl <- select(trawl_df, c('LCT', 'set_number'  ))
...
#let's do this with eDNA data 
eDNA_1$beta_detection_method = ifelse(eDNA_1$LCT %in% both , eDNA_1$beta_detection_method <- c("both eDNA/trawl"), eDNA_1$beta_detection_method <- c("only eDNA"))
eDNA_1 <- merge(eDNA, eDNA_1, by= c('set_number', 'LCT'))
eDNA_1 <- distinct(eDNA_1)

#let's do this with trawl data 
trawl_1$beta_detection_method = ifelse(trawl_1$LCT %in% both , trawl_1$beta_detection_method <- c("both eDNA/trawl"), trawl_1$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_2$beta_detection_method = ifelse(eDNA_2$LCT %in% both , eDNA_2$beta_detection_method <- c("both eDNA/trawl"), eDNA_2$beta_detection_method <- c("only eDNA"))
eDNA_2 <- merge(eDNA, eDNA_2, by= c('set_number', 'LCT'))
eDNA_2 <- distinct(eDNA_2)

#let's do this with trawl data 
trawl_2$beta_detection_method = ifelse(trawl_2$LCT %in% both , trawl_2$beta_detection_method <- c("both eDNA/trawl"), trawl_2$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_3$beta_detection_method = ifelse(eDNA_3$LCT %in% both , eDNA_3$beta_detection_method <- c("both eDNA/trawl"), eDNA_3$beta_detection_method <- c("only eDNA"))
eDNA_3 <- merge(eDNA, eDNA_3, by= c('set_number', 'LCT'))
eDNA_3 <- distinct(eDNA_3)

#let's do this with trawl data 
trawl_3$beta_detection_method = ifelse(trawl_3$LCT %in% both , trawl_3$beta_detection_method <- c("both eDNA/trawl"), trawl_3$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_4$beta_detection_method = ifelse(eDNA_4$LCT %in% both , eDNA_4$beta_detection_method <- c("both eDNA/trawl"), eDNA_4$beta_detection_method <- c("only eDNA"))
eDNA_4 <- merge(eDNA, eDNA_4, by= c('set_number', 'LCT'))
eDNA_4 <- distinct(eDNA_4)

#let's do this with trawl data 
trawl_4$beta_detection_method = ifelse(trawl_4$LCT %in% both , trawl_4$beta_detection_method <- c("both eDNA/trawl"), trawl_4$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_5$beta_detection_method = ifelse(eDNA_5$LCT %in% both , eDNA_5$beta_detection_method <- c("both eDNA/trawl"), eDNA_5$beta_detection_method <- c("only eDNA"))
eDNA_5 <- merge(eDNA, eDNA_5, by= c('set_number', 'LCT'))
eDNA_5 <- distinct(eDNA_5)

#let's do this with trawl data 
trawl_5$beta_detection_method = ifelse(trawl_5$LCT %in% both , trawl_5$beta_detection_method <- c("both eDNA/trawl"), trawl_5$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_7$beta_detection_method = ifelse(eDNA_7$LCT %in% both , eDNA_7$beta_detection_method <- c("both eDNA/trawl"), eDNA_7$beta_detection_method <- c("only eDNA"))
eDNA_7 <- merge(eDNA, eDNA_7, by= c('set_number', 'LCT'))
eDNA_7 <- distinct(eDNA_7)

#let's do this with trawl data 
trawl_7$beta_detection_method = ifelse(trawl_7$LCT %in% both , trawl_7$beta_detection_method <- c("both eDNA/trawl"), trawl_7$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_8$beta_detection_method = ifelse(eDNA_8$LCT %in% both , eDNA_8$beta_detection_method <- c("both eDNA/trawl"), eDNA_8$beta_detection_method <- c("only eDNA"))
eDNA_8 <- merge(eDNA, eDNA_8, by= c('set_number', 'LCT'))
eDNA_8 <- distinct(eDNA_8)

#let's do this with trawl data 
trawl_8$beta_detection_method = ifelse(trawl_8$LCT %in% both , trawl_8$beta_detection_method <- c("both eDNA/trawl"), trawl_8$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_9$beta_detection_method = ifelse(eDNA_9$LCT %in% both , eDNA_9$beta_detection_method <- c("both eDNA/trawl"), eDNA_9$beta_detection_method <- c("only eDNA"))
eDNA_9 <- merge(eDNA, eDNA_9, by= c('set_number', 'LCT'))
eDNA_9 <- distinct(eDNA_9)

#let's do this with trawl data 
trawl_9$beta_detection_method = ifelse(trawl_9$LCT %in% both , trawl_9$beta_detection_method <- c("both eDNA/trawl"), trawl_9$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_10$beta_detection_method = ifelse(eDNA_10$LCT %in% both , eDNA_10$beta_detection_method <- c("both eDNA/trawl"), eDNA_10$beta_detection_method <- c("only eDNA"))
eDNA_10 <- merge(eDNA, eDNA_10, by= c('set_number', 'LCT'))
eDNA_10 <- distinct(eDNA_10)

#let's do this with trawl data 
trawl_10$beta_detection_method = ifelse(trawl_10$LCT %in% both , trawl_10$beta_detection_method <- c("both eDNA/trawl"), trawl_10$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_11$beta_detection_method = ifelse(eDNA_11$LCT %in% both , eDNA_11$beta_detection_method <- c("both eDNA/trawl"), eDNA_11$beta_detection_method <- c("only eDNA"))
eDNA_11 <- merge(eDNA, eDNA_11, by= c('set_number', 'LCT'))
eDNA_11 <- distinct(eDNA_1)

#let's do this with trawl data 
trawl_11$beta_detection_method = ifelse(trawl_11$LCT %in% both , trawl_11$beta_detection_method <- c("both eDNA/trawl"), trawl_11$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_12$beta_detection_method = ifelse(eDNA_12$LCT %in% both , eDNA_12$beta_detection_method <- c("both eDNA/trawl"), eDNA_12$beta_detection_method <- c("only eDNA"))
eDNA_12 <- merge(eDNA, eDNA_12, by= c('set_number', 'LCT'))
eDNA_12 <- distinct(eDNA_12)

#let's do this with trawl data 
trawl_12$beta_detection_method = ifelse(trawl_12$LCT %in% both , trawl_12$beta_detection_method <- c("both eDNA/trawl"), trawl_12$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_13$beta_detection_method = ifelse(eDNA_13$LCT %in% both , eDNA_13$beta_detection_method <- c("both eDNA/trawl"), eDNA_13$beta_detection_method <- c("only eDNA"))
eDNA_13 <- merge(eDNA, eDNA_13, by= c('set_number', 'LCT'))
eDNA_13 <- distinct(eDNA_13)

#let's do this with trawl data 
trawl_13$beta_detection_method = ifelse(trawl_13$LCT %in% both , trawl_13$beta_detection_method <- c("both eDNA/trawl"), trawl_13$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_14$beta_detection_method = ifelse(eDNA_14$LCT %in% both , eDNA_14$beta_detection_method <- c("both eDNA/trawl"), eDNA_14$beta_detection_method <- c("only eDNA"))
eDNA_14 <- merge(eDNA, eDNA_14, by= c('set_number', 'LCT'))
eDNA_14 <- distinct(eDNA_14)

#let's do this with trawl data 
trawl_14$beta_detection_method = ifelse(trawl_14$LCT %in% both , trawl_14$beta_detection_method <- c("both eDNA/trawl"), trawl_14$beta_detection_method <- c("only trawl"))
...
#let's do this with eDNA data 
eDNA_16$beta_detection_method = ifelse(eDNA_16$LCT %in% both , eDNA_16$beta_detection_method <- c("both eDNA/trawl"), eDNA_16$beta_detection_method <- c("only eDNA"))
eDNA_16 <- merge(eDNA, eDNA_16, by= c('set_number', 'LCT'))
eDNA_16 <- distinct(eDNA_16)

#let's do this with trawl data 
trawl_16$beta_detection_method = ifelse(trawl_16$LCT %in% both , trawl_16$beta_detection_method <- c("both eDNA/trawl"), trawl_16$beta_detection_method <- c("only trawl"))
...
method_key <- method_key %>% #rename depth column 
  rename(
    gamma_detection_method = gamma_detection_method.y)

method_key <- select(method_key, -c('gamma_detection_method.x'))

#add weight + eDNA reads 
#select columns 

eDNA_envir <- select(eDNA_df, c('LCT','set_number', 'set_read_index', 'depth_eDNA', 'pabs_eDNA', 'north_south'))
...
trawl_envir <- select(trawl_df, c('LCT', 'set_number', 'weight_total_kg','pabs_trawl','depth_trawl', 'north_south'))
trawl_info <- select(trawl_df, c('LCT', 'set_number', 'weight_total_kg','pabs_trawl'))

q <- merge(method_key, trawl_info, by=c('LCT', 'set_number'))
q <- distinct(q) 

long <- merge(p, q, by = c("LCT", "set_number", "gamma_detection_method", "beta_detection_method"), all.x = T, all.y = T) %>% replace(is.na(.), 0)
#env <- merge()
