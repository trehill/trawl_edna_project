...
betadiff <-
ggplot(b1, aes(x = metric, y = value, group = set_number)) +
  geom_point() +
  geom_line() +
  ylab("dissimilarity") +
...
betadiff
ggsave("./Outputs/diversity/betadiff.png", 
       plot = betadiff,
       width = 2, height = 2, units = "in")


#models
# turnover:nestedness ratio
data$Jac_ratio <- ((data$Jac_turn + 0.01) / (data$Jac_nest + 0.01))
hist(data$Jac_ratio, breaks = 100)

t1 <- sqrt(data$Jac_ratio)
hist(t1)

#data$Jac_ratio <- log(data$Jac_ratio, base = 10)
#hist(data$Jac_ratio_trans)
