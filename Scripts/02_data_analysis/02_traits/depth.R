...
#plot Jaccard over Depth Difference + add set number as color 
plot <- ggplot(depth_beta,aes(depth_difference, Jac)) +
  geom_point(aes(colour = factor(set_number))) +
  geom_smooth(method='lm', se=FALSE, color='#00AFBB') +
  theme_minimal() +
  labs(x='Difference in Depth Between Sample Methods (m)', y='Jaccards Dissimilarity') +
  theme(plot.title = element_text(hjust=0.5, size=20, face='bold')) 
plot


ggsave("./Outputs/depth/jac_depth.png", 
       plot = plot,
       width = 10, height = 6, units = "in")

write_csv(depth_beta,
          here("Processed_data",
               "datasets",
               "depth_dif.csv")) 
