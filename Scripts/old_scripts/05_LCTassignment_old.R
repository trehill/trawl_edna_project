...
f2 <- merge(f1[c("ASV", "perc_ID", "species")], new_taxa[1:17], by = "species") #might need to change number inside []
...
spec_tax <- merge(spec, f2, by = c("ASV", "perc_ID"))%>%
  distinct()
y1 <- spec_tax[c(1:22)] #changed these brackets from Ben's code... not sure if it will cause issues 
...
length(data$ASV) #52
length(unique(data$ASV)) #52 #not duplicated 
length(unique(top10_occ$ASV)) #55 uh-oh, will need to fix this, some not accounted for 
...
#export and annotate: add "in_range" column in excel and annotate "y" if in 
#northeast pacific or tributaries, "n" if from other oceans (including western pacific)
#search in FishBase
#rename as below
#Raja binoculata --> does not come up in Fishbase, has different name than one listed (common name: Big Skate)
...
##################################manual editing required on next lines###########################################
r9 <- data.frame(all_species = unique(r8$all_species))                         #table of family groups
#r9$LCT <- c("Liparidae1")                            #add a group name (in order) for each row in r9, none in r9 
r10 <- merge(r8, r9, by = "all_species") %>%
  mutate(level = "family") %>%
  mutate(genus = LCT) %>%
  mutate(species = LCT) %>%
...
##################################manual editing required on next lines########################################### 
#q6$LCT <- c("Sebastes1")        #add a group name (in order) for each row in q6
#q7 <- merge(q5, q6, by = "all_species") %>% #no genera in 12su need to be edited 
#  mutate(level = "genus") %>%
#  mutate(species = LCT)
#d4 <- q7 %>%
#  .[c("ASV", "level", "LCT", "class", "order", "family", "genus", "species", "all_species")]
...
#check that all ASVs are accounted for and not duplicated - they should all be the same
length(data$ASV) #28
length(unique(data$ASV)) #28
length(unique(top10_occ$ASV)) #36  #uh oh 
